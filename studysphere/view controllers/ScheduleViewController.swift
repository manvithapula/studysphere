    ...
    var filterSchedules:[Schedule]{
      ...
      private var scheduleItems: [ScheduleItem]{
          ...
          for schedule in filterSchedules{
              let scheduleItem = ScheduleItem(iconName: schedule.topicType == TopicsType.flashcards ? "square.stack.3d.down.forward":"clipboard", title: schedule.title,subtitle: "", progress: (schedule.completed != nil) ? 1 : 0,topicType: schedule.topicType,topicId: schedule.topic)
      ...
      private func setupNavigationBar() {
          ...
          doneButton.tintColor = themeOrange
          navigationItem.rightBarButtonItem = doneButton
      }
      
      private func setupUI() {
          view.backgroundColor = themeDarkNavy // Updated
      ...
      private func updateSelectedDateView(index: Int) {
          dateViews.forEach { view in
              view.backgroundColor = themeDarkNavy.withAlphaComponent(0.8) // Updated
              if let stack = view.subviews.first as? UIStackView {
                  stack.arrangedSubviews.forEach { label in
                      if let label = label as? UILabel {
                          label.textColor = .white // Updated
                      }
                  }
              }
          }
          ...
          selectedView.backgroundColor = themeOrange // Updated
